...
class AutomatonGraphViewSemanticsAdapter {
  ...
  String _format(String text) =>
      formatEmptyStringNotation(text, emptyStringSymbol);

  String viewportLabel() {
    return _format(
      localizations.canvasViewportSemantics(
  ...
  String nodeLabel(
    ...
    final parts = <String>[
      localizations.canvasStateSemantics(
        _format(node.label.isEmpty ? node.id : node.label),
      ),
  ...
  String _edgeLabel(GraphViewCanvasEdge edge) {
    ...
    final from = (fromLabel?.isNotEmpty == true)
        ? _format(fromLabel!)
        : edge.fromStateId;
    final to = (toLabel?.isNotEmpty == true)
        ? _format(toLabel!)
        : edge.toStateId;
