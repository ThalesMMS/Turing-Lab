...
* Tablet, desktop and web layouts are canvas-first: the canvas fills the pane and the Simulation, Algorithms and Info panels stay collapsed behind the trailing icon rail of `WorkspaceDock`. Tap a rail icon to slide its panel in, tap it again (or the panel's close button) to give the width back to the canvas, and drag the divider to resize an open panel. Switching workspaces happens through the `WorkspaceSelector` drop-down at the leading edge of the app bar; there is no permanent navigation rail.【F:lib/presentation/widgets/workspace_dock.dart†L52-L225】【F:lib/presentation/widgets/workspace_selector.dart†L18-L140】【F:lib/presentation/widgets/automaton_workspace_scaffold.dart†L82-L160】
