...
void main() {
  group('JFLAP L-system codec', () {
    test('loads canonical fixture with JFLAP drawing semantics', () {
      ...
      expect(
          outcome, isA<CodecSuccess<InteroperableDocument<LSystemDocument>>>());
      final system =
      ...
      expect(system.commandMapping.commands['g'],
    ...
    test('round trip preserves advanced rewriting and turtle metadata', () {
      ...
      const codec = LSystemJflapCodec();
      final encoded = codec.encode(InteroperableDocument(
        document: system,
      ...
      expect(decoded.value.document.commandMapping.toJson(),
          system.commandMapping.toJson());
      expect(decoded.value.document.randomSeed, system.randomSeed);
      expect(decoded.value.document.ignoredContextSymbols,
...
<production><left>F(x)</left><right>F(x+1)</right></production></structure>''';
      final outcome = const LSystemJflapCodec().decode(_payload(source))
          as CodecSuccess<InteroperableDocument<LSystemDocument>>;
      ...
      expect(
        outcome.value.document.productions
            .where((production) => production.predecessor == 'A'),
        hasLength(2),
      );
      ...
      expect(
          outcome.value.document.unsupportedMetadata['jflapRules'], isNotEmpty);
      expect(
    ...
    test('rejects malformed, unsafe, and invalid numeric XML typingly', () {
      expect(const LSystemJflapCodec().decode(_payload('<structure>')),
      ...
      expect(
        const LSystemJflapCodec().decode(_payload(
...
<production><left>A</left><right>color(orange) g(2)</right></production>
      ...
      expect(
        outcome.value.document.productions.single.successor.symbols,
        ['color(orange)', 'g(2)'],
      );
    });
...
<parameter><name>polygonColor</name><value>dukeBlue</value></parameter>
      ...
      const codec = LSystemJflapCodec();
      final first = codec.decode(_payload(source))
          as CodecSuccess<InteroperableDocument<LSystemDocument>>;
  ...
  group('Turing Lab L-system JSON codec', () {
    test('canonical fixture round trips deterministically', () {
      ...
      final decoded = codec.decode(_fixture('json'))
          as CodecSuccess<InteroperableDocument<LSystemDocument>>;
      final first =
          codec.encode(decoded.value) as CodecSuccess<EncodedDocument>;
      final again = codec.decode(DocumentPayload(bytes: first.value.bytes))
          as CodecSuccess<InteroperableDocument<LSystemDocument>>;
    ...
    test('malformed and future schema fail without a partial document', () {
      final codec = LSystemJsonCodec();
      expect(codec.decode(_payload('{bad')),
          isA<CodecMalformed<InteroperableDocument<LSystemDocument>>>());
...
DocumentPayload _fixture(String extension) => DocumentPayload(
      bytes:
          File('test/fixtures/interoperability/l_system_canonical.$extension')
              .readAsBytesSync(),
